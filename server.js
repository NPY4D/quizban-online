...
function getDraftConfig(numPlayers) {
  // totalRounds ต้องเป็น multiple ของ numPlayers เพื่อให้ทุกคนได้ ban+pick เท่ากัน
  // แต่ไม่น้อยกว่า numPlayers (ทุกคนได้อย่างน้อย 1 ครั้ง)
  if (numPlayers <= 2) return { totalRounds: 4 }; // 2คน ได้คนละ 2 รอบ
  return { totalRounds: numPlayers };             // 3-6คน ได้คนละ 1 รอบ
}
  ...
  return {
    ...
    room: {
      ...
      totalQuestions: (function(){
        ...
        const t = room.picks.length * room.settings.qPerCat;
...
wss.on('connection', (ws) => {
  ws.on('message', async (raw) => {
    ...
    // ── REJOIN (reconnect after dropped WebSocket, e.g. mobile network blip) ──
    if (msg.type === 'rejoin') {
      const code = msg.code?.toUpperCase();
      const room = rooms.get(code);
      if (!room) return send(ws, { type: 'error', message: 'ห้องนี้ไม่มีแล้ว' });
      const player = room.players.find(p => p.id === msg.playerId);
      if (!player) return send(ws, { type: 'error', message: 'ไม่พบผู้เล่นนี้ในห้อง' });

      player.online = true;
      clients.set(ws, { roomCode: code, playerId: msg.playerId });
      send(ws, { type: 'joined', playerId: msg.playerId, code });
      send(ws, roomSnapshot(room));
      broadcastAll(code, roomSnapshot(room));

      // Re-send whatever question/reveal state is currently in flight,
      // so a reconnecting client doesn't get stuck with a blank screen.
      if (room.phase === 'game' && room.currentQuestion) {
        const answerer = getCurrentAnswerer(room);
        if (room.hasAnswered && room.revealData) {
          send(ws, { type: 'reveal_answer', ...room.revealData });
        } else if (room.currentQuestion.answer === -1) {
          if (room.hostId === msg.playerId) {
            send(ws, {
              type: 'request_host_question',
              catId: room.currentQuestion.catId, catName: room.currentQuestion.catName,
              questionIndex: room.questionIndex,
          ...
          send(ws, {
            type: 'new_question',
            question: { question: room.currentQuestion.question, choices: room.currentQuestion.choices, catName: room.currentQuestion.catName },
            questionIndex: room.questionIndex,
            totalQuestions: room.picks.length * room.settings.qPerCat,
            scores: room.scores, timePerQ: room.settings.timePerQ,
            catId: room.currentQuestion.catId,
            answererId: answerer.id, answererName: answerer.name, answererAvatar: answerer.avatar,
          });
        }
      }
...
function doAutoAdvance(room) {
  ...
  // จบเมื่อ questionIndex ครบ AND หารด้วย numPlayers ลงตัว (ทุกคนตอบเท่ากัน)
  const n = room.players.length;
  const total = room.picks.length * room.settings.qPerCat;
  // ปัดขึ้นให้หารด้วย n ลงตัว
  const fairTotal = Math.ceil(total / n) * n;

  if (room.questionIndex >= fairTotal) {
    room.phase = 'results';
    broadcastAll(room.code, roomSnapshot(room));
  } else {
    broadcastAll(room.code, roomSnapshot(room));
